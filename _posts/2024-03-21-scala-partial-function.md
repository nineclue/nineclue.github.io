...
정해진 범위의 인자를 받으면 함수마다 특정한 값을 반환하는데 범위에 해당하면 Option으로 감싸고 해당하지 않으면 None을 반환하여 범위에 해당하지 않는 것을 나타냅니다. 마지막 함수는 f3를 정의하기 위해 먼저 `PartialFunction` f3\_를 정의하고 lift 함수를 사용하였고 한줄로는 f3a와 같은 형태로 정의할 수도 있습니다.
